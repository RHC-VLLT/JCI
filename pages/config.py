...
def inject_css(bg_base64_url=None):
    ...
    .movie-card-xl {{ background-color: #0a0a0a; border: 1px solid #222; border-radius: 15px; height: 680px; position: relative; overflow: hidden; transition: all 0.4s ease; }}
    .movie-card-xl:hover {{ transform: translateY(-5px); border-color: #D7001D !important; }}
    
    .btn-main {{ background-color: #D7001D; padding: 14px 35px; text-decoration: none; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; border-radius: 4px; color: white !important; border: 1px solid #D7001D; }}
